...
/*   Updated: 2022/12/18 19:53:42 by lfranca-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	move_gamer(char **map, t_ray *rays, char *movement)
{
	...
	if (!ft_strncmp(movement, "forward", ft_strlen(movement)))
	{
	...
	else if (!ft_strncmp(movement, "backward", ft_strlen(movement)))
	{
...
int	event_key(int keycode, t_map *map)
{
	...
	if (keycode == KEY_W)
		move_gamer(map->map, &map->rays, "forward");
	if (keycode == KEY_S)
		move_gamer(map->map, &map->rays, "backward");
	if (keycode == KEY_A)
		move_gamer_to_sides(map->map, &map->rays, "to_left");
	if (keycode == KEY_D)
		move_gamer_to_sides(map->map, &map->rays, "to_right");
