...
/*   Updated: 2022/12/18 17:25:28 by lfranca-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	move_gamer(char **map, t_ray *rays, char *movement)
{
	...
	else if (!ft_strncmp(movement, "to_left", ft_strlen(movement)))
	{
		...
		else
		{
			...
			if (rays->gamer_angle == (float)0 || rays->gamer_angle == (float)(2*PI)) //0 pode ser (2*PI) - (2*PI)?
			{
				...
				if (map[collision->pos_y_sub_offset_y][collision->pos_x] == '0') //mudei esse pra sub_y?? pq senao ele ia reto contra a parede?
					rays->pos_y -= rays->deltax;
			...
			else
			{
				...
				if (map[collision->pos_y][collision->pos_x_add_offset_x] == '0') //antes tava 'sub', mas ele estava ultrapassando a parede à esquerda (cinza)..
					rays->pos_x += rays->deltay;  //antes era -= delta_x
				if (map[collision->pos_y_add_offset_y][collision->pos_x] == '0')
					rays->pos_y -= rays->deltax;	//antes era += delta_y
			}
	...
	else if (!ft_strncmp(movement, "to_right", ft_strlen(movement)))
	{
		if (rays->gamer_angle == (float)(PI/2) || rays->gamer_angle == (float)(3 * PI/2)) 
		{
			...
			// porque teremos que usar o deltay pra mover em x
			printf("Entrou no angulo do jogador\n");
				if (rays->gamer_angle == (float)(3 * PI/2) && (map[collision->pos_y][collision->pos_x_add_offset_x] == '0')) //sul ; alto
				{
					printf("Norte. DeltaY = %.1f | DeltaX = %.1f | Pos_x: %.2f\n", rays->deltay, rays->deltax, rays->pos_x); ///deltay -
					printf("posicao seguinte: %d | Posicao atual: %d\n", collision->pos_x_add_offset_x, collision->pos_x);
					rays->pos_x -= rays->deltay;
				}
		...
		else if (rays->deltax < 0) // ir para a esquerda olhando para oeste //---> south (PI) entra em deltax negativo! (-0.00) //colocar o angulo reto antes?
		{
			...
			{ //feito
				printf("Ele está olhando pro sul/90graus. Delta y: %.2f\n", rays->deltay);
				if (map[collision->pos_y][collision->pos_x_add_offset_x] == '0')
					rays->pos_x -= rays->deltay;  //antes era -= delta_x
				if (map[collision->pos_y_sub_offset_y][collision->pos_x] == '0')
					rays->pos_y += rays->deltax;	//antes era += delta_y		
		...
		{
			printf("Está indo pra direita e olhando pra east. Deltax é positivo: %.2f\n", rays->deltax);
			//ir pra esquerda se o deltax estiver POSITIVO (east)
			// o delta x é maior que zero (podendo ser 0.1 pode ser zero mais ainda eh decimal).
			// east == positivo
			if (rays->gamer_angle == (float)0 || rays->gamer_angle == (float)(2*PI))
			{ //FEITO
				// quando for positivo 0 e 360 eh quando o y | ELE CONSEGUE ENTRAR EM 0 E EM 360
				// printf("Está em angulo reto (0 ou 360)\n");
				printf("Delta Y é: %.3f\n", rays->deltay);
				...
				// olhando pro norte/270 graus //FEITO
				printf("DeltaY negativo\n.");
				printf("Delta Y é: %.3f | Delta X é: %.3f\n", rays->deltay, rays->deltax);
				if (map[collision->pos_y][collision->pos_x_add_offset_x] == '0')
					rays->pos_x -= rays->deltay;
				if (map[collision->pos_y_sub_offset_y][collision->pos_x] == '0')
					rays->pos_y += rays->deltax;
